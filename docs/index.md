---
title: AXTRACE knowledge base
description: База знаний AXTRACE - систематизация знаний по книгам, курсам и медицине
metadata:
  - property: 'og:type'
    content: 'website'
  - property: 'og:title'
    content: 'AXTRACE knowledge base'
  - property: 'og:description'
    content: 'База знаний AXTRACE - систематизация знаний по книгам, курсам и медицине'
  - property: 'og:url'
    content: 'https://axtrace.github.io/'
---

# AXTRACE knowledge base

Привет! Это попытка систематизировать некоторые мои знания.

## Основные разделы

{% list tabs %}

- Книги

  Список прочитанных книг с заметками и рецензиями

  [Перейти к книгам](books/overview.md)

- Курсы

  Пройденные онлайн-курсы и образовательные программы

  [Перейти к курсам](courses/overview.md)

- Медицина

  Информация о целиакии, вакцинации и других медицинских темах

  [Перейти к медицинским знаниям](health/overview.md)

{% endlist %}

## Дополнительные материалы

{% list tabs %}

- Вина

  Коллекция вин из Парфенона и дегустационные заметки

  [Перейти к винам](parfenon/index.md)

- Закладка

  Книги из программы "Закладка" от ещенепознер

  [Перейти к закладкам](zakladka/index.md)

- Ещенепознер

  Книги из выпусков #ещенепознер

  [Перейти к выпускам](yetnopozner_books/index.md)

- Заметки

  Разные заметки и размышления

  [Перейти к заметкам](notes/Peltzman_effect.md)

- Анекдоты

  Коллекция анекдотов

  [Перейти к анекдотам](anecdots/anecdotes.md)

{% endlist %}
